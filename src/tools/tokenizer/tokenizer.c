...
void	get_io_filename(t_minishell *msh, int i, int index)
{
	msh->filename_ls = (char **)malloc(sizeof(char *) * msh->command_count);
	msh->filename_ls[i]=ft_strdup(msh->command_table[i][index+1]);
